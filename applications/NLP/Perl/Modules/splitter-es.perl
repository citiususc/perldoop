...
open (VERB, "./lexicon/verbos-es.txt") or die "O ficheiro não pode ser aberto: $!\n";
binmode VERB,  ':utf8';
my @verb = <VERB>;
...
sub splitter {
 ...
 for (my $i=0;$i<=$#text;$i++) {
   ...
   my $tmp2;  
   my $found=0;;
###################separar verbos dos cliticos compostos oslo, noslo, selo, ... 
###FALTA TRATAR OS IMPERATIVOS!!!!! incluiremos em verbos-es.txt a lista de formas em imperativo!
 
   if ($token =~ /^($w+r)(nos|os|se)(lo|los|las|los)$/i) {
   ($verb,$tmp1,$tmp2 ) =  $token =~ /^($w+r)(nos|os|se)(lo|los|las|los)$/i;
  
   #print STDERR "---#$verb# - - #$tmp1# - #$tmp2#\n";
   if ($Verb{lowercase($verb)}  && $token =~ /(ár|ér|ír)(nos|os|se)(lo|los|las|los)$/) {
...
################separar cliticos simples de verbos  ###FALTA TRATAR OS IMPERATIVOS!!!!!
